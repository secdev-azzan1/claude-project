...
export function PlatformConnectionsPanel({ showHeading = true }: { showHeading?: boolean }) {
  ...
  const runTest = async (conn: PlatformConnection) => {
    ...
    try {
      ...
      if (result.health === "Healthy") toast.success(`${conn.name}: Healthy · Reachable`);
      else toast.error(`${conn.name}: test failed — Unreachable (connection refused)`);
    } catch (err) {
        ...
        TYPE_ORDER.map((type) => {
          ...
          return (
            <section key={type} className="space-y-3">
              ...
              {items.length === 0 ? (
                <div className="rounded-lg border border-dashed px-4 py-5 text-sm text-muted-foreground flex items-center justify-between gap-3">
                  <span>No {meta.label} connections yet.</span>
