...
async def main():
    ...
    max_wait_s = int(sys.argv[3]) if len(sys.argv) > 3 else 120
    ...
    async with httpx.AsyncClient(timeout=90) as client:
        ...
        print(f"[{flow_id}] polling (max {max_wait_s}s) for {topics}")
        start_t = time.time()
        ...
        while time.time() - start_t < max_wait_s:
            ...
            for t in topics:
                seen[t] = tc.get(t, 0)
            print(f"[{flow_id}]   t+{int(time.time()-start_t)}s queued={m.get('queued')} " + " ".join(f"{t}={seen[t]}" for t in topics))
