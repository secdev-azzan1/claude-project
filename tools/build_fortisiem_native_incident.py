"""
Native build of incident -- brand new entity, structurally independent of the device-detail chain
(JSON POST API, not XML CMDB pagination, per the approved plan). Triggered once per run off
maximum__run_metadata, not per-organization/per-device. Bounded 24h time window
(incidentLastSeen-ordered), paginated via start/size like rapid7's has_more/next_page loop.

JSON POST body is built natively via ReplaceText (Always Replace + EL substitution for
start/timeFrom/timeTo), not Groovy -- keeps the "one Groovy step, hash only" policy intact even
though this endpoint needs a request body, unlike the GET-only entities elsewhere in this flow.
"""
import sys
...
BODY_TEMPLATE = (
    '{"filters":{},"start":${page_start},"size":100,'
    '"timeFrom":${incident_time_from},"timeTo":${incident_time_to},'
    '"orderBy":"incidentLastSeen","descending":true,"fields":' + json.dumps(INCIDENT_FIELDS) + '}'
)
...
def main():
    ...
    init_page_id = create_processor(token, "org.apache.nifi.processors.attributes.UpdateAttribute", f"fortisiem.{ENTITY}__init_page", {
        "entity": ENTITY,
        "incident_time_to": "${now():toNumber()}",
        "incident_time_from": "${now():toNumber():minus(86400000)}",
        "page_start": "0",
        "kafka_topic": f"bronze.fortisiem.{ENTITY}__raw",
    }, UPDATE_ATTR_BUNDLE)

    build_body_id = create_processor(token, "org.apache.nifi.processors.standard.ReplaceText", f"fortisiem.{ENTITY}__build_body", {
    ...
    fetch_id = create_processor(token, "org.apache.nifi.processors.standard.InvokeHTTP", f"fortisiem.{ENTITY}__fetch", {
        "HTTP Method": "POST",
        "HTTP URL": "#{SOURCE_API_BASE}/pub/incident",
    ...
    extract_id = create_processor(token, "org.apache.nifi.processors.standard.EvaluateJsonPath", f"fortisiem.{ENTITY}__extract", {
        ...
        "incident_customer": "$.customer",
    }, STANDARD_BUNDLE, auto_terminate=["failure"])

    page_meta_id = create_processor(token, "org.apache.nifi.processors.standard.EvaluateJsonPath", f"fortisiem.{ENTITY}__page_meta", {
        "Destination": "flowfile-attribute",
        "resp_total": "$.total",
        "resp_start": "$.start",
        "resp_size": "$.size",
    ...
    has_more_id = create_processor(token, "org.apache.nifi.processors.standard.RouteOnAttribute", f"fortisiem.{ENTITY}__has_more", {
        "has_more": "${resp_start:plus(${resp_size}):lt(${resp_total})}",
    }, STANDARD_BUNDLE, auto_terminate=["unmatched"])

    next_page_id = create_processor(token, "org.apache.nifi.processors.attributes.UpdateAttribute", f"fortisiem.{ENTITY}__next_page", {
        "page_start": "${resp_start:plus(${resp_size})}",
    }, UPDATE_ATTR_BUNDLE)
    ...
    set_ids_id = create_processor(token, "org.apache.nifi.processors.attributes.UpdateAttribute", f"fortisiem.{ENTITY}__set_ids", {
        ...
        "api_path": "POST #{SOURCE_API_BASE}/pub/incident",
        "cursor_window": "${incident_time_from}_${incident_time_to}",
    }, UPDATE_ATTR_BUNDLE)
    ...
    headers_id = create_processor(token, "org.apache.nifi.processors.attributes.UpdateAttribute", f"fortisiem.{ENTITY}__set_public_headers", {
        ...
        "customer_tenant_organization": "${incident_customer}",
        ...
        "object_id_composite": "fortisiem:${incident_customer}:${entity}:${object_id}",
    ...
    create_connection(token, run_metadata_id, run_metadata_name, "PROCESSOR", init_page_id, f"fortisiem.{ENTITY}__init_page", "PROCESSOR", ["success"])
    create_connection(token, init_page_id, f"fortisiem.{ENTITY}__init_page", "PROCESSOR", build_body_id, f"fortisiem.{ENTITY}__build_body", "PROCESSOR", ["success"])
    create_connection(token, next_page_id, f"fortisiem.{ENTITY}__next_page", "PROCESSOR", build_body_id, f"fortisiem.{ENTITY}__build_body", "PROCESSOR", ["success"])
    create_connection(token, build_body_id, f"fortisiem.{ENTITY}__build_body", "PROCESSOR", fetch_id, f"fortisiem.{ENTITY}__fetch", "PROCESSOR", ["success"])
    ...
    create_connection(token, split_id, f"fortisiem.{ENTITY}__split", "PROCESSOR", page_meta_id, f"fortisiem.{ENTITY}__page_meta", "PROCESSOR", ["original"])
    create_connection(token, page_meta_id, f"fortisiem.{ENTITY}__page_meta", "PROCESSOR", has_more_id, f"fortisiem.{ENTITY}__has_more", "PROCESSOR", ["matched", "unmatched"])
    create_connection(token, has_more_id, f"fortisiem.{ENTITY}__has_more", "PROCESSOR", next_page_id, f"fortisiem.{ENTITY}__next_page", "PROCESSOR", ["has_more"])
