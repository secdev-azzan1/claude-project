...
PG_ID = os.environ.get("RAPID7_SECURADO_MAX_PG_ID", "1508dfff-01a0-1000-861c-4cbb8f1c946c")
CLIENT_ID = "codex-rapid7-securado-object-id-patch"
...
# Same 12 orphans as asyad, plus "scan" which only exists on the securado side.
ORPHAN_ENTITIES = [
    "asset_vulnerability_solution", "exploit", "malware_kit", "operating_system",
    "scan", "site_organization", "software", "solution", "tag", "tag_asset",
...
# ---------------------------------------------------------------------------
# NiFi REST helpers (same shape as tools/patch_rapid7_asyad_object_id_and_headers.py)
# ---------------------------------------------------------------------------
...
def run_once(wait_seconds=90):
    ...
    for name, p in procs.items():
        if name == "rapid7_securado.maximum__trigger" or "__replay__consume" in name:
            continue
        ent = nifi("GET", f"/nifi-api/processors/{p['id']}")
        if ent["component"].get("validationStatus") == "VALID":
            set_processor_state(p["id"], "RUNNING")

    trigger = procs["rapid7_securado.maximum__trigger"]
...
def remove_orphans():
    ...
    for e in ORPHAN_ENTITIES:
        subject = f"bronze.rapid7_securado.{e}__raw.avro-value"
        schema_results.append(delete_schema_subject(subject))
        connector_name = f"bronze.rapid7_securado.{e}__raw.avro__iceberg"
...
def apply_raw_ingest_ts_cast(entity):
    procs = processors_by_name()
    prefix = f"rapid7_securado.{entity}"
...
def apply_entity(entity):
    ...
    headers_id = create_processor(
        ...
        mid_x, mid_y,
        {
            ...
            "object_id_composite": "rapid7:rapid7_securado:${entity}:${object_id}:${'content_SHA-256'}",
        },
        [],
    )
...
def fix_schema(entity):
    subject = f"bronze.rapid7_securado.{entity}__raw.avro-value"
...
def verify():
    ...
    invalid = [p["component"]["name"] for p in procs if p["component"]["name"].startswith("rapid7_securado.") and p["component"].get("validationStatus") not in ("VALID", None)]
    not_stopped = [p["component"]["name"] for p in procs if p["component"]["name"].startswith("rapid7_securado.") and p.get("status", {}).get("aggregateSnapshot", {}).get("runStatus") not in ("Stopped", "Disabled")]
    ...
    for entity in ENTITIES:
        ...
        cast = by_name.get(f"{prefix}__raw__cast_ingest_ts")
        if not cast:
            mismatches.append(f"{prefix}__raw__cast_ingest_ts MISSING")
        elif cast["config"]["properties"].get("Script Body") != RAW_INGEST_TS_CAST_SCRIPT:
            mismatches.append(f"{prefix}__raw__cast_ingest_ts script mismatch")
    ...
    orphan_connectors_remaining = [c for c in connectors if any(f"bronze.rapid7_securado.{e}__raw.avro__iceberg" == c for e in ORPHAN_ENTITIES)]
