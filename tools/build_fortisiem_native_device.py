"""
Native rebuild of the `device` entity in fortisiem.maximum_useful, matching rapid7_asyad's exact
architecture: set_ids -> hash (one Groovy step) -> set_public_headers -> set_metadata (native
UpdateRecord) -> dedupe_key -> dedupe (native DetectDuplicate) -> raw__publish, plus
...
Does NOT wire the interface/processor/storage/installed_software/device_custom_property cascade
here -- those get their own new dedicated-endpoint fetch chains in separate scripts, branched off
device_detail__extract_key directly (same "fan out before dedup" fix already applied to
organization->list_devices__fetch), not off this script's output.
"""
import sys
...
def main():
    ...
    set_ids_id = create_processor(token, "org.apache.nifi.processors.attributes.UpdateAttribute", f"fortisiem.{ENTITY}__set_ids", {
        "entity": ENTITY,
        "object_id": "${naturalId}_${organization__attr_id}_${access_ip}",
        "api_path": "GET #{SOURCE_API_BASE}/cmdbDeviceInfo/device?organization=${org_name}&ip=${access_ip}&loadDepend=true",
    ...
    # 4) connections
    extract_name = "fortisiem.maximum__device_detail__extract_key"
