          ...
          <CardContent className={schemaWorkspaceLayout.artifactContent}>
            <div className={schemaWorkspaceLayout.artifactList}>
              {isLoading ? (
                ...
                <EmptyState
                  icon={FileCode2}
                  title={artifacts.length === 0 ? "No schemas yet" : "No matches"}
                >
                  {artifacts.length === 0
                    ? "Run a schema ceremony from a kafka+connect block, or start a library template."
      ...
      <Dialog open={!!ceremonyPickerFor} onOpenChange={(open) => !open && setCeremonyPickerFor(null)}>
        <DialogContent className="max-h-[80vh] max-w-lg overflow-y-auto">
          ...
          {ceremonyTargets.length === 0 ? (
            <EmptyState icon={FileCode2} title="No kafka+connect blocks yet">
              Add one in a flow first — it is the only block a schema can be approved against.
            </EmptyState>
          ) : (
            <div className="space-y-4">
