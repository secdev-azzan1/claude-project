...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn(
        "border-b border-border/50 transition-colors data-[state=selected]:bg-primary-muted hover:bg-muted/45",
        className,
      )}
      {...props}
    />
...
// Column headers become small caps rather than another row of body-weight text —
// with 13px body copy below them, a 14px medium header did not read as a header.
