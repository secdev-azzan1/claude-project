...
// Edge sheets get an inner corner radius, the way a macOS panel that slides in
// from a screen edge is rounded on the side facing the content and square
// against the edge it came from.
const sheetVariants = cva(
  "fixed z-50 flex flex-col gap-4 border-border/60 bg-popover p-6 shadow-2xl transition ease-spring data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-300",
  {
    variants: {
      side: {
  ...
  ({ side = "right", className, children, ...props }, ref) => (
    <SheetPortal>
      ...
      <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
        {children}
        <SheetPrimitive.Close className="absolute right-4 top-4 flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-accent hover:text-foreground focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-ring/35 disabled:pointer-events-none">
          <X className="h-4 w-4" />
