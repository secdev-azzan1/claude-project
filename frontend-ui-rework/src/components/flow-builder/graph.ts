...
// Loosened ~10% over the previous constants — closer to what auto-layout tools
// converge on for a tree this shape (e.g. dagre's own commonly-tuned spacing
// for labelled nodes: nodesep/ranksep well above its 50/50 defaults). The
// forest-tree walk below stays hand-rolled on purpose: branches in this model
// never merge, so a general graph-layout library has nothing to solve here
// that this O(n) walk doesn't already.
const X_GAP = 320;
const Y_GAP = 124;
